 ...
 * Optimized: keeps positions in worker memory, only receives view matrix.
 * Uses 16-bit radix sort for O(n) performance.
 */
...
self.onmessage = function(e) {
    ...
    // Store positions once (avoid 240MB copy per sort)
    if (type === "init") {
        storedPositions = e.data.positions;
        storedCount = e.data.count;
        return;
    }
